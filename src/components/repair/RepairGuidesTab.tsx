...
interface Props {
  onGoToDiagnostic: (symptomSlug: string, category: string) => void;
}

export default function RepairGuidesTab({ onGoToDiagnostic }: Props) {
  ...
  const handleBack = () => {
    if (selectedSymptom) setSelectedSymptom(null);
    ...
    <div className="space-y-6 mt-4">
      ...
      {selectedSymptom && (
        <RepairGuideStep
          ...
          onGoToDiagnostic={() =>
            onGoToDiagnostic(selectedSymptom.slug, selectedSymptom.category)
          }
        />
      )}
    </div>
