...
import { Search, Plus, Edit, Boxes, PieChart } from "lucide-react";
...
export default function AdminModels() {
  ...
  const fetchData = async () => {
    try {
      const [modelsRes, specsRes, adsCountRes, mappedAdsRes] = await Promise.all([
      ...
      if (!modelsRes.data || modelsRes.data.length === 0) {
        setModels([
          { id: 1, brand: 'NVIDIA', name: 'RTX 4090', family: 'Ada Lovelace', hardware_categories: { name: 'GPU' }, aliases: ['GeForce RTX 4090', '4090'], updated_at: new Date().toISOString() },
          { id: 2, brand: 'AMD', name: 'RX 7900 XTX', family: 'RDNA 3', hardware_categories: { name: 'GPU' }, aliases: ['Radeon RX 7900 XTX'], updated_at: new Date().toISOString() },
        ]);
